"""M3-07 golden tests: SqliteVecVectorStore persistence / owner filter / delete."""
...
def test_upsert_and_search(tmp_path: Path) -> None:
    store = SqliteVecVectorStore(tmp_path / "vectors", dim=8)
    v = _vec("knowledge base content")
...
def test_persistence_after_restart(tmp_path: Path) -> None:
    ...
    v = _vec("persisted content")
    store1 = SqliteVecVectorStore(path, dim=8)
    store1.upsert("alice", "doc-1", [("c1", v)])
    store2 = SqliteVecVectorStore(path, dim=8)
...
def test_dim_mismatch_validation(tmp_path: Path) -> None:
    ...
    with pytest.raises(ValidationError):
        store.upsert("alice", "doc-1", [("c1", [0.0, 0.0])])
    with pytest.raises(ValidationError):
